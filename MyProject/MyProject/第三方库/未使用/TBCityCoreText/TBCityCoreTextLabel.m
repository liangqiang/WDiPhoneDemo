...
- (void)drawRect:(CGRect)rect {
    ...
    //补误差值，
    _textHeight = ceil(self.frame.size.height-lineOrigin.y) + 5;
    
    if (self.fixToCenterY) {
        CGRect frame = self.frame;
        //-4是为了纠偏，不是很准
        frame.origin.y = _originY + (self.frame.size.height - 4 - _textHeight) /2;
        self.frame = frame;
    }
    ...
    for (int i = 0; i < CFArrayGetCount(lines); i++) {
        ...
        for (int j = 0; j < CFArrayGetCount(runs); j++) {
            ...
            if (imageUrl) {
                ...
                imageDrawRect.origin.y = self.frame.size.height - lineOrigin.y - imageDrawRect.size.height + 2;
...
//    CGPoint origins[CFArrayGetCount(lines)];
    CGPoint *origins  = malloc(CFArrayGetCount(lines));
    //获取每行的原点坐标
